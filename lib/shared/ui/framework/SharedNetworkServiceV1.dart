...
class SharedNetworkServiceV1 implements SharedNetworkService {
  ...
  Future<void> sendLocation(
      Map<String, dynamic> location, Demande demande) async {
    if (socket.connected) {
  ...
  Future<void> getLocation(Demande demande, User? auth) async {
    if (socket.connected) {
      ...
      socket.on('socketStartCourse', (resp) {
        if (resp["demande"]["id"] == demande.id) {
          startCourse.value = resp;
        }
      });
      socket.on('socketCloseCourse', (resp) {
        if (resp["demande"]["id"] == demande.id) {
          closeCourse.value = resp;
        }
      });
    }
  }
